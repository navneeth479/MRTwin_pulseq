experiment_id = 'exB04_Z01.8_FISP_jonaspetersen'
...
zoom = 1
_,gz2,_ = make_sinc_pulse(flip_angle=-90 * math.pi / 180,delay = 0.002, phase_offset=-90*np.pi/180, duration=3e-3,slice_thickness=slice_thickness, apodization=0.5, time_bw_product=4, system=system)

# Define other gradients and ADC events
gx = make_trapezoid(channel='x', flat_area=(Nread*1.5), flat_time=3e-3, system=system)
adc = make_adc(num_samples=Nread, duration=2e-3, phase_offset=0*np.pi/180, system=system,delay=gx.rise_time)
gx_pre = make_trapezoid(channel='x', area=-gx.area / (2*1.5), duration=1e-3, system=system)
...
# CONSTRUCT SEQUENCE
# ======
print(calc_duration(gx)-calc_duration(gp))
for ii in range(-Nphase//2, Nphase//2):  # e.g. -64:63
    ...
    gp_= make_trapezoid(channel='y', area=-ii, duration=1e-3, system=system, delay=calc_duration(gx)-calc_duration(gp))
    seq.add_block(adc,gx,gp_)
    seq.add_block(rf2)
    #seq.add_block(gzr2)
    ### Nur Test
