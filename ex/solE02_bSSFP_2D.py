experiment_id = 'E02_bSSFP_2D_Nav_64_fruit3'
# Scanner experiment
...
# Define rf events
rf1, g180,_ = make_sinc_pulse(flip_angle=15 * math.pi / 180, duration=1e-3,slice_thickness=slice_thickness, apodization=0.5, time_bw_product=4, system=system)
# rf1, _= make_block_pulse(flip_angle=90 * math.pi / 180, duration=1e-3, system=system)

# Define other gradients and ADC events
gx = make_trapezoid(channel='x', flat_area=Nread/fov*zoom, flat_time=1e-3, system=system)
...
# ======
sdel=1e-0
# half flip angle prep pulse
rf0, gz,gzr = make_sinc_pulse(flip_angle=15/2 * math.pi / 180, duration=1e-3,slice_thickness=slice_thickness, apodization=0.5, time_bw_product=4, system=system)
seq.add_block(rf0,gz)
seq.add_block(gzr)

seq.add_block(make_delay(0.5e-3)) # half TR of rf1
...
for ii in range(-Nphase//2, Nphase//2):  # e.g. -64:63
    ...
    seq.add_block(rf1,g180)
    gp= make_trapezoid(channel='y', area=ii/fov*zoom, duration=1e-3, system=system)
