experiment_id = ' exC02_spinecho_to_RARE_2D'

# To generate a Turbo Spin Echo (TSE)  or  RARE (Rapid Acquisition relaxation enhanced)
...
seq.add_block(rf1)
seq.add_block(make_delay(2.5e-3 - calc_duration(rf2) + 2 * rf2.dead_time))  # add rf1 with 90° flip_angle 
seq.add_block(gx_pre)

for ii in range(-Nphase//2,Nphase//2):    
    gp= make_trapezoid(channel='y', area=-ii, duration=1e-3, system=system)
    gp_neg= make_trapezoid(channel='y', area=ii, duration=1e-3, system=system)
    seq.add_block(gx_pre,gp) 
    seq.add_block(make_delay(2e-3)) 
    seq.add_block(rf2)    

    seq.add_block(adc,gx)
    seq.add_block(gx_pre,gp_neg) # opposite polarity gp not used cause of 180 rf pulse 
